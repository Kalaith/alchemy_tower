...
pub(crate) fn draw_journal_brews_tab_view(
    ...
    for entry in &view.entries {
        draw_ui_text(&entry.title, x + 20.0, brew_y, 20.0, dark::TEXT_BRIGHT);
        brew_y += 20.0;
        draw_ui_text(&entry.state_line, x + 20.0, brew_y, 17.0, dark::TEXT_DIM);
        brew_y += 18.0;
        draw_wrapped_text(
            &entry.recap,
            x + 20.0,
        ...
        if let Some(effects_text) = &entry.effects_text {
            draw_ui_text(effects_text, x + 580.0, brew_y - 2.0, 18.0, dark::TEXT_DIM);
            if let Some(traits_text) = &entry.traits_text {
                draw_ui_text(traits_text, x + 580.0, brew_y + 20.0, 18.0, dark::TEXT_DIM);
            }
        }
        brew_y += 40.0;
        if let Some(best_brew_text) = &entry.best_brew_text {
            draw_ui_text(best_brew_text, x + 20.0, brew_y, 18.0, dark::TEXT_DIM);
            brew_y += 22.0;
        }
        if let Some(formula_text) = &entry.formula_text {
            draw_ui_text(formula_text, x + 20.0, brew_y, 18.0, dark::TEXT_DIM);
            brew_y += 22.0;
        }
