...
pub(crate) fn draw_alchemy_preview_panel_view(
    ...
    match &view.state {
        AlchemyPreviewPanelState::EmptySelection => draw_empty_selection(x, y),
...
fn draw_empty_selection(x: f32, y: f32) {
    draw_text(
...
fn draw_resolved_brew_preview_view(
    ...
    let detail_y = if preview.has_recipe && !preview.failure_reasons.is_empty() {
        process_y + 8.0
    } else {
        process_y + 4.0
    };
...
fn draw_failure_reasons_view(preview: &AlchemyResolvedPreviewView, x: f32, y: f32) -> f32 {
    ...
    next_y += 20.0;
    for reason in &preview.failure_reasons {
        draw_text(
            &format!("- {reason}"),
            x + 12.0,
            next_y,
