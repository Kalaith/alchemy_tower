...
def station(kind, size):
    ...
    if "cauldron" in kind:
        ...
        for ox in [-20, -7, 6, 19]:
            d.line((cx + ox, cy + 12, cx + ox, cy - 6), fill=rgb("#8fb173"), width=2)
            d.ellipse(box(cx + ox, cy - 9, 7, 7), fill=rgb("#f2b8a4"))
        d.ellipse(box(cx + 14, cy - 20, 6, 4), fill=rgb("#f7d9c4"))
...
def icon(item, size, world=False):
    ...
    if item in {
        ...
        c = {
            ...
            "hardwinter_draught": "#e8c8aa", "sluicewater_tonic": "#a6c6ca",
        }.get(item, "#d2c1ff")
